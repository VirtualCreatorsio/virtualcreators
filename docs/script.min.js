...
const assetPathPrefix = '' // Disable path prefix logic - use hardcoded paths in HTML
...
const pdfPath = isDutchLocale ? `../assets/${pdfFileName}` : `assets/${pdfFileName}`
link.href = pdfPath
...
// Skip entirely for Dutch pages - use hardcoded paths in HTML
if (isDutchLocale) {
return
}
...
? 'VIRTUAL CREATORS Service & Pricing Guide Dutch.pdf'
: 'VIRTUAL CREATORS Service & Pricing Guide.pdf'
const pdfPath = `assets/${pdfFileName}#toolbar=0&navpanes=0&scrollbar=0${mobileParams}`
...
// Only set PDF iframe source for English pages to avoid overriding Dutch hardcoded paths
if (!isDutchLocale) {
setPDFIframeSource()
}
}
